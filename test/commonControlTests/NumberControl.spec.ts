...
suite('NumberControl e2e tests', () => {
    ...
    suite(
        'NumberControl e2e tests - NumberControl with expectation function and validation functions',
        () => {
            const TEST_CONTROL_ID = 'NumberSelectorWithoutValidationExpectation';
    ...
    suite('NumberControl e2e tests - NumberControl with custom validation and expectation', () => {
        ...
        class NumberControlManager extends ControlManager {
            createControlTree(): Control {
                return new NumberControl({
                    ...
                    validation: (state) =>
                        (state.value! >= 0 && state.value! <= 50) || {
                            renderedReason: 'the value must be between zero and fifty inclusive',
                        },
                    confirmationRequired: (state, input) =>
                        state.value !== undefined && ![1, 2, 3, 4, 5, 6].includes(state.value),
                });
            }
        }
        ...
        test('first number disaffirmed, unusable repeated number happens, third number expected', async () => {
            ...
            await testE2E(requestHandler, [
                ...
                'A: My mistake. How many?',
                'U: Change the number',
                TestInput.of(GeneralControlIntent.of({ action: $.Action.Change, target: $.Target.Number })),
                'A: How many?',
