...
suite('== game_e2e.ts ==', () => {
    suite('== demo game UI, directly hitting coreProcess() ==', () => {
        ...
        test('request class value (by list)', async () => {
            ...
            const result1 = await simpleInvoke(
                rootControl,
                TestInput.of(GeneralControlIntent.of({ action: $.Action.Set, target: $$.Target.CharClass })),
            );
        ...
        test('custom intent setting multiple values via MultiValueContainerControl', async () => {
            ...
            const input = TestInput.of(
                IntentBuilder.of('MySetPlayerIntent', {
                    name: 'Mike',
                    class: 'human',
                    age: '20',
                }),
    ...
    suite('== demo game UI, e2e with SPI, serde  ==', () => {
        test('e2e', async () => {
            ...
            expect(response.prompt).equals(
                "Sorry, Alexa is not a valid choice because playerName Validation Failed. What is your avatar's name?",
            );
        ...
        petContainer
            .addChild(
                new ListControl({
                    ...
                    validation: [
                        (state, input) =>
                            this.getPetSpecies().includes(state.value!)
                                ? true
                                : { renderedReason: 'petSpecies validation failed' },
                    ],
                    prompts: {
            ...
            .addChild(
                new ValueControl({
                    ...
                    prompts: {
                        requestValue: (act, input) =>
                            `What shall we name your ${
                                (input.controls[$$.ID.PetSpecies] as ListControl).state.value ?? 'pet'
                            }?`,
                    },
...
class PlayerContainer extends ContainerControl {
    playerNameControl = new ValueControl({
        ...
        validation: [
            (state, input) =>
                ['Mike', 'Dave'].includes(state.value!)
                    ? true
                    : { renderedReason: 'playerName Validation Failed' },
        ],
        prompts: {
    ...
    playerClassControl = new ListControl({
        ...
        validation: [
            (state, input) =>
                this.getPlayerClass().includes(state.value!)
                    ? true
                    : { renderedReason: 'playerClass validation Failed' },
        ],
        prompts: {
        ...
        if (InputUtil.isIntent(input, 'MySetPlayerIntent')) {
            ...
            if (intent.slotResolutions.name !== undefined) {
                ...
                this.playerNameControl.setValue(
                    nameSlotResolution.slotValue,
                    nameSlotResolution.isEntityResolutionMatch,
                );
            }
            ...
            if (intent.slotResolutions.class !== undefined) {
                ...
                this.playerClassControl.setValue(
                    classSlotResolution.slotValue,
                    classSlotResolution.isEntityResolutionMatch,
                );
            }
            return;
        }
