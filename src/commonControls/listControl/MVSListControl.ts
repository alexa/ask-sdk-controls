...
export class MVSListControl extends Control implements InteractionModelContributor {
    ...
    constructor(props: MVSListControlProps) {
        ...
        if (props.slotType === AmazonBuiltInSlotType.SEARCH_QUERY) {
            throw new Error(
                'AMAZON.SearchQuery cannot be used with MVSListControl due to the special rules regarding its use. ' +
                    ...
                    'Use a custom intent to manage SearchQuery slots or create a regular slot for use with MVSListControl.',
            );
        }
    ...
    private validate(input: ControlInput) {
        ...
        for (const validationFunction of listOfValidationFunc) {
            ...
            if (validationResult !== true) {
                log.debug(
                    `MVSListControl.validate(): validation failed. Reason: ${JSON.stringify(
                        validationResult,
                        null,
                        2,
                    )}.`,
    ...
    private askElicitationQuestion(
        ...
        if (allChoices === null) {
            throw new Error('MVSListControl.listItemIDs is null');
        }
