...
The central concept is that a skill can be built from _Controls_ that encapsulate portions of
...
4. **_Initiative phase (if needed)_**. Determines a chain of control to _take the
   initiative_ and continue the dialog. This phase runs if the handle phase did not
...
6. **_Shutdown phase_** Collects and serializes the state of each control and returns
...
3. Find an interesting test or suite, mark it as `test.only()` then step into it with `F5`
